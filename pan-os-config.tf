  ...
  provisioner "local-exec" {
    command = <<EOF
               ...
               PATH=${path.module}/venv/bin:/usr/local/bin:$HOME/.local/bin:$PATH
                ...
                ANSIBLE_ROLES_PATH="${path.module}/roles" ansible-playbook -i ${path.module}/pan-os-ansible/inventory.ini -e ansible_python_interpreter=`which python` ${path.module}/pan-os-ansible/playbook.yml
              EOF
  }
