  ...
  provisioner "local-exec" {
    command = <<EOF
               ...
               PATH=${path.module}/venv/bin:/usr/local/bin:$HOME/.local/bin:$PATH
                ...
                pip install ansible==${var.pip_ansible_version} netaddr==${var.pip_netaddr_version} pan-python xmltodict pandevice requests requests_toolbelt dnspython lxml
                virtualenv ${path.module}/venv
                ...
                ANSIBLE_ROLES_PATH="${path.module}/roles" ansible-playbook -i ${path.module}/pan-os-ansible/inventory.ini -e ansible_python_interpreter=/usr/bin/python2 ${path.module}/pan-os-ansible/playbook.yml
              EOF
  }
