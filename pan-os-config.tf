...
resource "null_resource" "panos_settings" {
  provisioner "local-exec" {
    command = <<EOF
                ...
                pip3 install ansible==${var.pip_ansible_version} netaddr==${var.pip_netaddr_version} pan-python requests requests_toolbelt dnspython lxml selinux libselinux-python
                virtualenv --system-site-packages ${path.module}/venv
