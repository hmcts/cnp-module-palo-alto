...
resource "null_resource" "panos_settings" {
  provisioner "local-exec" {
    command = <<EOF
                ...
                pip install ansible==${var.pip_ansible_version} netaddr==${var.pip_netaddr_version} pan-python pandevice xmltodict pytest-runner jsonschema==2.6.0 ipaddress requests requests_toolbelt dnspython lxml
